...
class KafkaHandler(logging.Handler):
    def __init__(self, host, *args, **kwargs):
        ...
        set.kafka_producer = KafkaProducer(bootstrap_servers=host)
    ...
    def emit(self, record):
        ...
        self.kafka_producer.send(str(record.name).encode('utf-8') + '.json', json_dump.encode('utf-8'))
        self.kafka_producer.send('all.json', json_dump.encode('utf-8'))
        self.kafka_producer.send(str(record.name).encode('utf-8') + '.txt', memoryview.encode('utf-8'))
        self.kafka_producer.send('all.txt', message.encode('utf-8'))
        self.flush()
    
    def flush(self):
        set.kafka_producer.flush()
